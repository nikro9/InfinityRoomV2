...
# Importamos la función del gráfico desde un módulo centralizado
from src.ui_components import create_main_chart 

# --- CONFIGURACIÓN DE LA PÁGINA ---
st.set_page_config(layout="wide", page_title="Altcoin Pivots")
STRATEGY_CONFIG = config.STRATEGIES["ALTCOIN_PIVOTS"]

st.title("📈 Infinity Room - Pivots para Altcoins")

# --- SELECTOR DE ACTIVO ---
selected_asset = st.selectbox(
    "Selecciona un Activo para Monitorear",
    STRATEGY_CONFIG["asset_list"],
    help="El worker de Altcoins analiza todos estos activos en segundo plano."
)

st.markdown(f"Mostrando análisis para **{selected_asset}** con un SML de **{STRATEGY_CONFIG['sml_timeframe']}**.")
...
while True:
    # Creamos las claves de Redis dinámicamente basadas en el activo seleccionado
    redis_prefix = f"{STRATEGY_CONFIG['redis_prefix']}:{selected_asset.replace('/', '')}"
    ...
    with placeholder_status.container():
        reasoning_text = status_data.get('reasoning', f'Esperando al worker...') if status_data else f'Esperando al worker...'
        st.info(f"**Análisis de {selected_asset}:** {reasoning_text}")
    ...
    with placeholder_chart.container():
        if chart_data and 'data' in chart_data and 'columns' in chart_data:
            ...
            st.warning(f"Esperando datos del worker para {selected_asset}...")
